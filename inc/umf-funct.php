...
function showMessage( $message, $errormsg = false ) {
	if ( $errormsg ) { echo '<div id="message" class="error">';} else {echo '<div id="message" class="updated fade">';}
...
function upload_files_field( $order_id ) {
	...
	if ( isset( $_FILES ) && isset( $_POST['upload'] ) && $_POST['upload'] ) {
		...
		foreach ( $_FILES as $key => $umf_file ) {
			...
			if ( $_FILES[ $key ]['size']< ($filesize*1024*1024) ) {$sizeallow=true; } else {$sizeallow=false; $upload_error.= '<li">' . sprintf( __( 'The file "%s" is to big.', 'woocommerce-umf' ), $umf_file['name'] ) . '</li>';}
